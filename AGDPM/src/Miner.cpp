...
void construct_conditional_ds(FPTree& fptree, DS& transactions, Item& item) {
	...
	while (path_starting_fpnode != nullptr) {
		...
		if (curr_path_fpnode->parent != nullptr) {
			...
			Transaction tr0;
			tr0.first = transformed_prefix_path;
			tr0.second = 0;
			Transaction tr1;
			tr1.first = transformed_prefix_path;
			tr1.second = 1;
...
void branch_and_bound(FPTree& fptree, double& max_IG, Feature& best, double bound, Itemset& alpha, double& number_of_transactions, double& transactions_labeld_1_counter, int k, int l) {
	...
	for (auto it = fptree.header_table.begin(); it != fptree.header_table.end(); ++it) {
		...
		//Insert beta to queue if not full or if full and IGbeta is better than current lowest IG in queue
		if (IG_beta > max_IG) {
			max_IG = IG_beta;
...
double get_initial_bound(FPTree & fptree, double& number_of_transactions, double& transactions_labeld_1_counter, int k, std::vector<Feature> & singletons) {
	PatternSet FS;
	if (fptree.header_table.size() == 0) {
		return 0;
	}
...
void singleton_iteration(FPTree & fptree, PatternSet & FS, double& number_of_transactions, double& transactions_labeld_1_counter, int k, std::vector<Feature> & singletons, double bound, int l) {
	...
	std::for_each(std::execution::par, std::begin(singletons), std::end(singletons), [&](Feature f) {
		...
		double IGub = InfoGainUB((transactions_labeld_1_counter) / number_of_transactions, ((double)(f.second.first) + (double)f.second.second) / number_of_transactions);
		...
		if (bound < IGub) {
			...
			if (cond_transactions.size() > 0) {
				FPTree cond_fptree = FPTree{ cond_transactions,fptree.min_sup };
				branch_and_bound(cond_fptree, max_IG, best, bound, alpha, number_of_transactions, transactions_labeld_1_counter, k, l);
			}
		}
...
PatternSet mine(DS transactions, double min_sup_percent, int k, int l,
	...
	uint64_t min_sup = (transactions.size() * min_sup_percent) / 100;
	PatternSet FS;
	Itemset best_pat;
	...
	inital_IG_bound = get_initial_bound(fptree, number_of_transactions, transactions_labeld_1_counter, k, singletons);
	Itemset alpha;
	singleton_iteration(fptree, FS, number_of_transactions, transactions_labeld_1_counter, k, singletons, inital_IG_bound, l);
