...
DS read_transactions_file(std::string& transactions_file_path, std::map<int, std::string>& item_by_id,
	std::map<std::string, int>& id_by_item, double& origins_labeld_1_counter, double& number_of_origins, InputErrorType& err) {
	std::string str;
	DS parsed_ds;
	char dim = ',';
	int curr_item_id = 0;
	...
	if (transactions_file.is_open()) {
		while (std::getline(transactions_file, str)) {
			...
			Label lable = std::stoi(str.substr(str.length() - 1));
			...
			if (itemset.size() > 0) {
				parsed_ds.push_back(Transaction(transaction_origin_name,std::pair<Itemset,Label>(itemset, lable)));
				if (origins.find(transaction_origin_name) == origins.end()) {
					origins.insert(transaction_origin_name);
					number_of_origins++;
					if (lable == 1) origins_labeld_1_counter++;
				}
			}
		}
...
void recommend_redundant(std::vector<std::pair<Itemset, size_t>>& potentially_redundant, std::vector<Feature>& final_set_vector, std::vector<Itemset>& completed_patterns) {
	...
	for (int i = 0; i < completed_patterns.size(); ++i) {
		for (int j = 0; j < completed_patterns.size(); ++j) {
			if (j == i)
				continue;
			if (is_pattern_in_pattern(potentially_redundant[i].first, completed_patterns[j])) {
...
void make_outfile(PatternSet& final_set, std::string& output_file_name,
	...
	double origins_labeld_1_counter, double origins_labeld_0_counter) {
	...
	for (auto feature = final_set_vector.rbegin(); feature != final_set_vector.rend(); ++feature) {
		...
		outfile << "Complete pattern(" << pattern.size() << "): ";
		item = pattern.begin();
		outfile << item_by_id[*item];
		item++;
		for (; item != pattern.end(); item++) {
		...
		outfile << "Support (by genome not transaction) in label 0: " << feature->second.first << "/" << origins_labeld_0_counter << std::endl;
		outfile << "Support (by genome not transaction) in label 1: " << feature->second.second << "/" << origins_labeld_1_counter << std::endl;
		j++;
