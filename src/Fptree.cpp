...
void count_frequencies_itemset(Itemset& itemset, Origin& origin, std::map<Item, std::set<Origin>>& origins_by_item,
	std::map<Item, uint64_t>& origin_frequency_by_item, std::map<Item, uint64_t>& frequency_by_item) {

	for (const Item& item : itemset) {
		inc_frequency(frequency_by_item, item);
...
void count_frequencies_transactions(std::vector<Transaction>& transactions, uint64_t& number_of_origins, std::map<Item, std::set<Origin>>& origins_by_item,
	...
	for (auto it = transactions.begin(); it != transactions.end(); it++) {
		...
		count_frequencies_itemset(it->itemset, origin, origins_by_item, origin_frequency_by_item, frequency_by_item);
	}

	number_of_origins = origins.size();
}

void impose_min_sup(std::vector<Transaction>& transactions, uint64_t& min_sup, uint64_t& number_of_origins,
	std::map<Item, uint64_t>& origin_frequency_by_item, std::map<Item, uint64_t>& frequency_by_item) {
	uint64_t transactions_size = transactions.size();
	for (auto it = transactions.begin(); it != transactions.end(); it++) {
		it->itemset.erase(
			std::remove_if(it->itemset.begin(), it->itemset.end(),
			[&origin_frequency_by_item,&frequency_by_item, min_sup, number_of_origins, transactions_size](const Item & x) {
				uint64_t origin_freq_x = origin_frequency_by_item[x];
				uint64_t freq_x = frequency_by_item[x];
				return (origin_freq_x < min_sup || freq_x == transactions_size);
			}
		), it->itemset.end());
	}
}
	...
	min_sup(min_sup)
{
	...
	count_frequencies_transactions(transactions, number_of_origins, origins_by_item, origin_frequency_by_item, frequency_by_item);

	impose_min_sup(transactions, min_sup, number_of_origins, origin_frequency_by_item, frequency_by_item);
